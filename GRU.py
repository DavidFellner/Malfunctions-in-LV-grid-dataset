...
class GRU(nn.Module):
    def __init__(self, input_size, output_size, hidden_dim, n_layers):
        ...
        self._gru = nn.GRU(input_size, hidden_dim, n_layers).to(self._device)
    ...
    def forward(self, x):
        ...
        if x.device == torch.device("cpu"):
            self._gru = self._gru.to(torch.device("cpu"))
            self._fc = self._fc.to(torch.device("cpu"))
            out, hidden = self._gru(x, hidden)
            out = self._fc(out)
        else:
            self._gru = self._gru.to(self.choose_device())
                ...
                for i, (sequences, labels, raw_seq) in enumerate(train_loader):
                    ...
                    if configuration['decision criteria'] == 'majority vote':
                        if configuration['calibration rate'] == 1:
                            start_voting_outputs = int((configuration['calibration rate']) * output.size()[1]) - 1      #equal to only using the last output
                        else:
        ...
        if X is not None:
            ...
            for i, (input_sequences, labels, raw_seq) in enumerate(test_loader):
                ...
                if configuration['decision criteria'] == 'majority vote':
                    ...
                    voting_outputs = torch.stack([i[start_voting_outputs:] for i in
                                                  outputs])  # choose last n outputs of timeseries to do majority vote
                    relevant_outputs = voting_outputs.to(self._device)
